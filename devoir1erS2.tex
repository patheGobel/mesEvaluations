...
\subsection*{b) Etudions la continuite de f sur son $D_{f}$: (1 pt $\times$ 2)}
\[ f(x) = \begin{cases} 
...
Montrer que $f$ est continue en 0 mais n'est pas en 0 
...
\item[d)]$f(x)=(x^{2}+1)(5x-7)$\quad\quad $e)f(x)=\frac{-2x+1}{3x+5}$\quad\quad $f)f(x)=x+3+\frac{4}{x-1}$\quad\quad $g)f(x)=\sqrt{2x+1}$
\end{itemize}
...
\subsection*{a) Compléter les formules $\alpha$(0,75 pt $\times$ 2+0,5 )}
