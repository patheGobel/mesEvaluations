...
1)
	...
	b)Montrer que $\forall x>1, h(x)=\ln(-xe^{-x}+e^{-1}) $ puis montrer que la droite $y=x-1$ d’équation est une asymptote à $\Gamma$ au voisinage de $+\infty$. \textbf{1pt}
...
Donc $h'(x)=\frac{1-x}{f(x)}$

D'après la réponse à la question 2), $\forall x\in \mathbb{R}\setminus\left\lbrace 1\right\rbrace $, $f(x)>0$

Donc le signe de $h(x)$ dépend de celui du numérateur $1-x$
\begin{itemize}
\item[•] Si $x\in \left]-\infty; 1\right[$, f'(x)>0 donc f est croissante
\item[•] Si $x\in \left]1; +\infty\right[$, f'(x)<0 donc f est décroissante
\end{itemize}
...
\draw (-19,7) node[anchor=north west] {$-\infty$};
\draw (-13.3,6.5) node[anchor=north west] {$1$};
\draw (-13.1,3.5) node[anchor=north west] {$+\infty$};
\draw (-14.3,3.5) node[anchor=north west] {$+\infty$};
\draw (-8,7) node[anchor=north west] {$+\infty$};
%signe de la dérivé
\draw (-17,5.3) node[anchor=north west] {$+$};
...
\draw [->,line width=2pt] (-18,-1) -- (-13.5,3);
\draw (-18.8,-1) node[anchor=north west] {$-\infty$};

\draw [->,line width=2pt] (-12.5,3) -- (-8,-1);
\draw (-8,-1) node[anchor=north west] {$1$};
\end{tikzpicture}
\end{document}